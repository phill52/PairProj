		...
		switch (filterObj.label) {
			case "skills":
				projects = projects.filter((project) => {
					...
					return skillNames.includes(filterObj.value);
				});
				break;

			case "difficulty":
				projects = projects.filter(
					(project) => project.difficulty === filterObj.value
				);
				break;
...
export default function ProjectCard() {
	...
	const projects = useMemo(() => {
		let filtered = filterProjects(mockProjects, filtersSelected);
				...
				<Card key={project.id} className="flex flex-col justify-between">
					...
					<CardContent className="space-y-3">
						...
						<div className="text-sm text-gray-600 space-y-1">
							<p>
								{project.ProjectMembership.length} members
							</p>
